/*
EasyAPI REST Services API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 1.1.1
Contact: easyapi@easydns.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the RegDataExtraPARIS type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &RegDataExtraPARIS{}

// RegDataExtraPARIS struct for RegDataExtraPARIS
type RegDataExtraPARIS struct {
	// Link to Paris community
	ParisNexus string `json:"paris_nexus"`
}

type _RegDataExtraPARIS RegDataExtraPARIS

// NewRegDataExtraPARIS instantiates a new RegDataExtraPARIS object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewRegDataExtraPARIS(parisNexus string) *RegDataExtraPARIS {
	this := RegDataExtraPARIS{}
	this.ParisNexus = parisNexus
	return &this
}

// NewRegDataExtraPARISWithDefaults instantiates a new RegDataExtraPARIS object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewRegDataExtraPARISWithDefaults() *RegDataExtraPARIS {
	this := RegDataExtraPARIS{}
	return &this
}

// GetParisNexus returns the ParisNexus field value
func (o *RegDataExtraPARIS) GetParisNexus() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ParisNexus
}

// GetParisNexusOk returns a tuple with the ParisNexus field value
// and a boolean to check if the value has been set.
func (o *RegDataExtraPARIS) GetParisNexusOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ParisNexus, true
}

// SetParisNexus sets field value
func (o *RegDataExtraPARIS) SetParisNexus(v string) {
	o.ParisNexus = v
}

func (o RegDataExtraPARIS) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o RegDataExtraPARIS) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["paris_nexus"] = o.ParisNexus
	return toSerialize, nil
}

func (o *RegDataExtraPARIS) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"paris_nexus",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varRegDataExtraPARIS := _RegDataExtraPARIS{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varRegDataExtraPARIS)

	if err != nil {
		return err
	}

	*o = RegDataExtraPARIS(varRegDataExtraPARIS)

	return err
}

type NullableRegDataExtraPARIS struct {
	value *RegDataExtraPARIS
	isSet bool
}

func (v NullableRegDataExtraPARIS) Get() *RegDataExtraPARIS {
	return v.value
}

func (v *NullableRegDataExtraPARIS) Set(val *RegDataExtraPARIS) {
	v.value = val
	v.isSet = true
}

func (v NullableRegDataExtraPARIS) IsSet() bool {
	return v.isSet
}

func (v *NullableRegDataExtraPARIS) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableRegDataExtraPARIS(val *RegDataExtraPARIS) *NullableRegDataExtraPARIS {
	return &NullableRegDataExtraPARIS{value: val, isSet: true}
}

func (v NullableRegDataExtraPARIS) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableRegDataExtraPARIS) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


